        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-[1fr_0.75fr]">
          <section aria-labelledby="intro-title" className="rounded-3xl bg-white p-6 ring-1 ring-slate-200 sm:p-8">
            ...
            <div className="mt-8 rounded-2xl bg-amber-50 p-5 text-sm leading-6 text-amber-950">
              ...
              <p className="mt-1">방문하거나 신청하기 전 <a href={event.sourceUrl} target="_blank" rel="noreferrer" {...analyticsData("source_link_clicked", { event_slug: event.slug, event_category: event.category, link_position: "verification_notice", source_type: "primary" })} className="font-bold underline">{event.sourceName} 원문<span className="sr-only">(새 창)</span></a>을 확인하세요.</p>
            </div>
          </section>
          <section aria-labelledby="location-title" className="self-start rounded-3xl bg-teal-900 p-6 text-white sm:p-8">
            ...
            {mapLinks ? (
              <div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 text-sm font-bold text-cyan-100">
                <a href={mapLinks.naver} target="_blank" rel="noreferrer" {...analyticsData("map_link_clicked", { event_slug: event.slug, event_category: event.category, link_position: "event_location", map_method: mapLinks.hasVerifiedCoordinates ? "verified_location" : "search" })} className="underline underline-offset-4">{mapLinks.hasVerifiedCoordinates ? "네이버 지도에서 위치 확인" : "네이버 지도에서 검색"} <span className="sr-only">(새 창)</span></a>
              </div>
            ) : (
