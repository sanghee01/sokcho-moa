...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{ status?: string }> }) {
  ...
  const [allEvents, places] = await Promise.all([getAdminEvents(), getAdminPlaces()]);
    ...
    <AdminShell email={admin.email}>
      <section className="grid gap-4 sm:grid-cols-3">
        {Object.entries(statusLabels).map(([key, label]) => (
          <Link key={key} href={`/admin?status=${key}`} className={`rounded-2xl p-5 ring-1 ${status === key ? "bg-teal-800 text-white ring-teal-800" : "bg-white ring-slate-200"}`}>
            <p className="text-sm font-bold opacity-75">{label}</p>
            <p className="mt-2 text-3xl font-black">{allEvents.filter((row) => row.review_status === key).length}</p>
          </Link>
        ))}
      </section>
      ...
      <section className="mt-10" aria-labelledby="admin-events-title">
        <div className="mb-4 flex items-center justify-between gap-3">
          ...
          <Link href="/admin/events/new" className="rounded-xl bg-teal-800 px-4 py-2.5 text-sm font-bold text-white">신규 행사</Link>
        </div>
      ...
      <section className="mt-10" aria-labelledby="admin-places-title">
        <div className="mb-4 flex items-center justify-between"><h2 id="admin-places-title" className="text-2xl font-black text-slate-950">주변 명소 {places.length}곳</h2><Link href="/admin/places/new" className="text-sm font-bold text-teal-700 underline">명소 등록</Link></div>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{places.map((row) => <Link key={String(row.id)} href={`/admin/places/${row.id}`} className="rounded-2xl bg-white p-4 ring-1 ring-slate-200 hover:ring-teal-400"><p className="text-xs font-bold text-teal-700">{String(row.category)}</p><p className="mt-1 font-black">{String(row.name)}</p><p className="mt-2 text-xs text-slate-500">{row.is_published ? "공개" : "비공개"}</p></Link>)}</div>
      </section>
    </AdminShell>
  );
}
...
function AdminSetup() {
  return (
    <main id="main-content" className="mx-auto max-w-3xl px-4 py-16 sm:px-6">
      ...
      <Link href="/admin/login" className="mt-8 inline-block rounded-2xl bg-teal-800 px-5 py-3 font-bold text-white">로그인 화면 확인</Link>
    </main>
  );
}
