...
- 제품 변경 커밋 `bf8bbaa`, 이를 포함해 배포된 최신 통합 `main` 커밋 `e5973c8`. GitHub combined status와 Vercel Production deployment는 모두 `success`다.
- 운영 도메인: `https://sokcho-moa.vercel.app`.
...
- 위치 카드는 데스크톱 최대 244px, 모바일 최대 228px다. 무비 나잇은 각각 200px·184px로 기존 288px 지도 예약 영역이 남지 않았다.
...
- 최신 `e5973c8` production 재감사 실패 수는 데스크톱 0/17, 모바일 0/17이다.
