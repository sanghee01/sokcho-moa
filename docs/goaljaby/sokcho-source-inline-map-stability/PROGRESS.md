...
모든 운영 행사의 공식 원문과 위치를 실제 콘텐츠까지 검증하고, 상세 화면에서 지도 미리보기와 SDK를 제거한 채 장소·주소와 정확한 단일 장소 또는 공식 주소가 확인된 영역 장소의 외부 지도 링크를 제공하며, 다중 장소에는 원문 주소 확인을 안내하고 이미지 대체 화면과 관리자 상태 변경 UI를 안정화한다.

## 현재 마일스톤

마일스톤 5 — 전체 검증·배포·운영 감사 완료
...
- 지도 미리보기 제거 제품·문서 커밋 `41b72ec`를 `main`에 반영한 뒤, 이를 포함하는 최신 통합 `main` `e5973c8`의 Vercel production 배포 성공 확인
- production 공개 예정 행사 17건의 상세 위치 카드를 데스크톱·모바일로 전수 순회해 지도 SDK·iframe·로딩/fallback 박스·가로 넘침·콘솔 오류가 모두 0건임을 확인
- production 단일·영역 장소 16건의 네이버 외부 위치/검색 링크와 새 창·analytics·접근성 계약을 확인하고, 주소가 서로 다른 원데이캠프 1건은 단일 오해 링크 없이 원문 주소 확인 안내가 표시됨을 확인
- 무비 나잇 위치 카드는 데스크톱 200px·모바일 184px로 빈 288px 예약 영역이 없고, 최신 production 전체 위치 카드 최대 높이는 데스크톱 244px·모바일 228px임을 확인
...
- 기준: 지도 제거와 병행 반응형·로딩 개선을 모두 포함한 최신 원격 main `e5973c8`
...
- pnpm test:e2e: 통과 (desktop/mobile 14 tests)
...
- git diff --check: 통과
- Vercel production `main`: `e5973c8`, GitHub/Vercel Production 상태 success, 운영 도메인 `https://sokcho-moa.vercel.app`
- production 데스크톱 공개 행사 상세: 17/17 감사, 지도 SDK 요청 0, iframe 0, 지도 로딩·실패 문구 0, 가로 넘침 0, 콘솔·page error 0, 실패 0
- production 모바일 공개 행사 상세: 17/17 감사, 지도 SDK 요청 0, iframe 0, 지도 로딩·실패 문구 0, 가로 넘침 0, 콘솔·page error 0, 실패 0
- 외부 링크: 정확한 단일·영역 장소 16건 정상, 서로 다른 주소의 다중 장소 1건은 정확성 안내 정상
- 데스크톱·모바일 위치 카드 스크린샷을 직접 시각 검사해 텍스트 겹침·잘림·빈 지도 영역이 없음을 확인
```
...
| 14 | production 17건 브라우저 순회 첫 실행 | Playwright CLI의 제한된 함수 환경에 전역 `URL` 객체가 없어 제품 페이지 방문 전 중단 | 문자열 기반 동일 출처 경로 결합으로 바꿔 전체 순회를 재실행, 제품 실패가 아님 |
| 15 | 외부 링크 전수 검사 | `네이버 지도에서 위치 확인`만 찾는 검사기가 좌표 없는 검색 링크 1건을 누락으로 오판 | 실제 접근성 트리에서 `네이버 지도에서 검색` 링크를 확인하고 도메인·속성 기준으로 검사 수정 |
| 16 | 다중 장소 외부 링크 검사 | 서로 다른 주소 4곳을 가진 원데이캠프의 의도적 링크 생략을 실패로 오판 | 임의 대표 장소 금지 계약에 따라 원문 주소 확인 안내를 정상 예외로 명시하고 17/17 재검증 |
| 17 | 독립 완료 게이트의 AC-2 문서 대조 | production의 `속초해수욕장 일원` 공식 주소 검색 링크와 `정확한 단일 장소`로만 좁게 쓴 문서 표현이 불일치 | 사용자의 `현재 화면에서 미리보기만 제거` 결정과 기존 구현에 맞춰 정확한 단일 장소 또는 공식 주소가 확인된 영역 장소까지 허용한다고 계약을 명확히 하고 최신 production 17/17을 재검증 |

## 현재 가장 안정적인 상태

지도 SDK와 미리보기 제거는 제품 커밋 `bf8bbaa`로 구현되고 이를 포함하는 최신 통합 `main` `e5973c8`이 Vercel Production에 성공 배포됐다. 최신 트리의 로컬 필수 검증과 운영 데스크톱·모바일 17/17 감사를 통과했고 지도 SDK·iframe·빈 fallback·가로 넘침·콘솔 오류는 0건이다. 백업·원문·위치 좌표·검수 상태는 변경하지 않았다.

## 다음 단계

최종 운영 감사 기록을 커밋·반영하고 Goal 완료 감사를 수행한다. 제품 기능과 운영 데이터에 남은 변경 작업은 없다.

## 리스크 / 블로커

- 지도 플랫폼 계정·Client ID는 더 이상 완료 전제가 아니며, production 데스크톱·모바일 전수 감사에서 카카오·네이버 SDK 요청 0건을 증명했다.
...
- 인라인 지도 재도입은 비용·한도·제공자 정책을 새로 승인하는 미래의 별도 Goal이며 현재 코드에 SDK 환경 계약을 되살리지 않는다.
...
- 주소가 서로 다른 다중 장소에는 하나의 오해할 검색 링크를 만들지 않고 원문에서 개별 주소를 확인하도록 안내한다.
