...
모든 운영 행사의 공식 원문과 위치를 실제 콘텐츠까지 검증하고, 상세 화면에서 지도 미리보기와 SDK를 제거한 채 장소·주소와 정확한 단일 장소의 외부 지도 링크를 제공하며, 다중 장소에는 원문 주소 확인을 안내하고 이미지 대체 화면과 관리자 상태 변경 UI를 안정화한다.
...
- 지도 미리보기 제거 제품·문서 커밋 `41b72ec`를 `main`에 반영하고 Vercel production 자동 배포 완료
...
- 무비 나잇 위치 카드는 데스크톱 200px·모바일 184px로 빈 288px 예약 영역이 없고, 전체 위치 카드 최대 높이는 데스크톱 244px·모바일 260px임을 확인
...
- git diff --check: 통과
- Vercel production `main`: `41b72ec`, 운영 도메인 `https://sokcho-moa.vercel.app`
...
지도 SDK와 미리보기 제거는 제품 커밋 `bf8bbaa`, 최종 배포 커밋 `41b72ec`로 production에 반영됐고 로컬 필수 검증과 운영 데스크톱·모바일 17/17 감사를 통과했다. 지도 SDK·iframe·빈 fallback·가로 넘침·콘솔 오류는 0건이다. 백업·원문·위치 좌표·검수 상태는 변경하지 않았고, 원래 작업트리의 병행 브랜딩 파일도 건드리지 않았다.
